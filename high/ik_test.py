#!/usr/bin/env python3
"""
Unitree G1 + RealSense D435i 웹 스트리밍 + 클릭 → IK 로봇 제어
클릭한 위치로 로봇 팔 이동
"""
...
app = FastAPI(title="G1 RealSense IK Control")
...
<body>
    <h1>G1 RealSense IK Control</h1>
    <div class="subtitle">클릭 -> 좌표 계산 -> 로봇 이동</div>
    ...
    <div id="container">
        ...
        <div id="coords">
            ...
            <div class="coord-row">
                <span class="label">깊이:</span>
                <span class="value" id="depth">-</span>
                <span class="unit">m</span>
            </div>
            
            <h3 class="section-title" style="margin-top:25px;">로봇 제어</h3>
            <div id="ik-output">target_pos = [x, y, z]</div>
            <div style="margin-top:10px; font-size:18px;">
                <span class="label">선택된 팔:</span>
            ...
            <div class="param-row">
                <span class="param-label">손바닥 R,P,Y (도):</span>
                <input type="number" id="rpy-roll" class="param-input" value="0" step="5">
                <input type="number" id="rpy-pitch" class="param-input" value="0" step="5">
                <input type="number" id="rpy-yaw" class="param-input" value="0" step="5">
            </div>
    ...
    <script>
            ...
            fetch('/get_coords?u=' + u + '&v=' + v)
                ...
                .then(data => {
                    ...
                    document.getElementById('ik-output').textContent = 
                        '[' + data.torso_x.toFixed(4) + ', ' + 
                        data.torso_y.toFixed(4) + ', ' + data.torso_z.toFixed(4) + ']';
                    
                    const armSide = data.torso_y >= 0 ? '왼팔' : '오른팔';
                    document.getElementById('arm-side').textContent = armSide;
                    
                    document.getElementById('btn-move').disabled = false;
                    setStatus('좌표 준비됨 - 이동 버튼 클릭', 'ready');
                });
        }
        ...
        function moveRobot() {
            ...
            const roll = parseFloat(document.getElementById('rpy-roll').value) || 0;
            const pitch = parseFloat(document.getElementById('rpy-pitch').value) || 0;
            const yaw = parseFloat(document.getElementById('rpy-yaw').value) || 0;
        ...
        function addHistory(coords) {
            ...
            item.textContent = '[' + coords.torso_x.toFixed(3) + ', ' + 
...
async def get_coords(u: int = 0, v: int = 0):
    ...
    region = depth_image[v-2:v+3, u-2:u+3]
    valid = region[region > 0]
    
    if len(valid) == 0:
        return JSONResponse({"error": "유효하지 않은 깊이"})
    
    depth_mm = float(np.median(valid))
    torso_x, torso_y, torso_z = pixel_to_torso_coords(u, v, depth_mm)
    
    print(f"픽셀({u:3d},{v:3d}) -> [{torso_x:.4f}, {torso_y:.4f}, {torso_z:.4f}]")
